...
import site.hackery.parametric.exceptions.*
import site.hackery.parametric.parser.ArgumentParser
import site.hackery.parametric.utils.PeekableIterator
import site.hackery.parametric.utils.splitExceptingQuotes

class CommandManager(registerDefaultParsers: Boolean = true) {
    ...
    private fun parseArgumentsForBranch(branch: Command.CommandBranch, args: List<String>): Array<Any> {
        ...
        if (parsers.isEmpty() && args.size > startIndex) {
            throw ParsingException(InvalidArgumentCount(branch.name
                    ?: "<blank>"))
        }
